...
\begin{enumerate}
  \item Пять семисегментных дисплеев:
  \begin{itemize}
...
Было выделено 8 состояний $S = \{S_0, S_1, S_2, S_3, S_4, S_5, S_6, S_7\}$, где
\begin{itemize}
  ...
  \item $S_7$ -- состояние секундомера. На индикаторах -- минуты и секунды секундомера. В этом состоянии секундомер не отсчитывает время.
\end{itemize}
...
\begin{table}[h!]
    ...
    \hline
    $\mathbf{S_4}$ & $S_4$ & $S_5$ & $S_7$ \\
    \hline
    $\mathbf{S_5}$ & $S_5$ & $S_4$ & $S_0$ \\
    \hline
    $\mathbf{S_6}$ & $S_6$ & $S_6$ & $S_0$\\
    \hline
    $\mathbf{S_7}$ & $S_4$ & $S_7$ & $S_0$\\
    \hline
    \end{tabular}
\end{table}
...
\begin{table}[h!]
  ...
  \hline
  $\mathbf{S_4}$ & $z_5$ & $z_4$ & $z_0$ \\
  \hline
  $\mathbf{S_5}$ & $z_5$ & $z_4$ & $z_5$ \\
  \hline
  $\mathbf{S_6}$ & $z_0$ & $z_0$ & $z_6$\\
  \hline
  $\mathbf{S_7}$ & $z_0$ & $z_5$ & $z_5$\\
  \hline
  \end{tabular}
\end{table}

\newpage
На Рис.~\ref{img:automaton} представлен смоделированный конечный автомат.
...
\begin{table}[H]
  ...
  ab & $s_4:$ stopwatch running & $s_7:$ stopwatch & $z_0$ \\
  \hline
  ...
  % s7
  a  & $s_7:$ stopwatch & $s_4:$ stopwatch running & $z_0$ \\
  b  & $s_7:$ stopwatch & $s_7:$ stopwatch & $z_5$ \\
  ab & $s_7:$ stopwatch & $s_0:$ time & $z_5$ \\
  \hline
  \end{tabularx}
\end{table}
...
\textit{Демультиплексор} --- это цифровое устройство, которое принимает один входной сигнал и направляет его на один из нескольких выходов в зависимости от управляющих сигналов. На Рис.~\ref{img:demux} представлена логическая схема 3-to-8 демультиплексора.

\begin{figure}[H]
   \centering
   \includegraphics[scale=0.6]{demux.png}
   \caption{Логическая схема 3-to-8 демультиплексора}
   \label{img:demux}
\end{figure}
...
На Рис. 10 представлена общая функциональная схема часов.

\newpage
\includepdf[fitpaper, pages=1]{interface.pdf}
\refstepcounter{figure}
\newpage

Интерфейс часов состоит из 5 семисегментных дисплеев и 2 диодов для отображения режима работы часов.
...
\newpage
\includepdf[fitpaper, pages=1]{time_counter.pdf}
\refstepcounter{figure}
\newpage
...
\newpage
\includepdf[fitpaper, pages=1]{stopwatch_counter.pdf}
\refstepcounter{figure}
\newpage
...
\newpage
\includepdf[fitpaper, pages=1]{F.pdf}
\refstepcounter{figure}
\newpage
